package oceanicvoid.thedeeperdark.dimensions.teleporter;
...
import oceanicvoid.thedeeperdark.blocks.DeeperDarkPortalBlock;
...
public class DeeperDarkTeleporter implements ITeleporter {
    protected final ServerLevel level;
    final int height = 7;
    final int width = 7;

    public DeeperDarkTeleporter(ServerLevel worldIn) {
        this.level = worldIn;
    }
    ...
    public Optional<BlockUtil.FoundRectangle> getExistingPortal(BlockPos pos) {
        ...
        Optional<PoiRecord> optional = poiManager.getInSquare((poiType) ->
                poiType.get() == (ModPOIs.DEEPER_DARK_PORTAL.get()), pos, 64, PoiManager.Occupancy.ANY).sorted(Comparator.<PoiRecord>comparingDouble((poi) ->
    ...
    public Optional<BlockUtil.FoundRectangle> makePortal(BlockPos pos, Direction.Axis axis) {
        ...
        for(BlockPos.MutableBlockPos blockpos$mutable1 : BlockPos.spiralAround(pos, width*width, Direction.EAST, Direction.SOUTH)) {
            ...
            if (worldborder.isWithinBounds(blockpos$mutable1) && worldborder.isWithinBounds(blockpos$mutable1.move(direction, 1))) {
                ...
                for(int l = j; l >= 0; --l) {
                    ...
                    if (this.level.isEmptyBlock(blockpos$mutable1)) {
                        ...
                        if (l + height - 1 <= dimensionLogicalHeight) {
                            int j1 = i1 - l;
                            if (j1 <= 0 || j1 >= 3) {
                                blockpos$mutable1.setY(l);
            ...
            for(int l1 = -1; l1 < 2; ++l1) {
                for(int k2 = 0; k2 < 2; ++k2) {
                    for(int i3 = -1; i3 < 3; ++i3) {
                        BlockState blockstate1 = i3 < 0 ? ModBlocks.ACTIVATED_DEEPSLATE.get().defaultBlockState() : Blocks.AIR.defaultBlockState();
        ...
        if (this.level.dimension() != Level.OVERWORLD) {
            for (int k1 = -1; k1 < width - 1; ++k1) {
                for (int i2 = -1; i2 < height - 1; ++i2) {
                    if (k1 == -1 || k1 == width - 2 || i2 == -1 || i2 == height - 2) {
                        mutablePos.setWithOffset(blockpos, k1 * direction.getStepX(), i2, k1 * direction.getStepZ());
                        this.level.setBlock(mutablePos, ModBlocks.ACTIVATED_DEEPSLATE.get().defaultBlockState(), 3);
                    }
                }
            }

            BlockState kaupenPortal = ModBlocks.DEEPER_DARK_PORTAL.get().defaultBlockState().setValue(DeeperDarkPortalBlock.AXIS, axis);

            for (int j2 = 0; j2 < width - 2; ++j2) {
                for (int l2 = 0; l2 < height - 2; ++l2) {
        ...
        return Optional.of(new BlockUtil.FoundRectangle(blockpos.immutable(), width-2, height-2));
    }
    ...
    public PortalInfo getPortalInfo(Entity entity, ServerLevel level, Function<ServerLevel, PortalInfo> defaultPortalInfo) {
        boolean destinationIsUG = level.dimension() == ModDimensions.THEDEEPERDARK_KEY;
        if (entity.level.dimension() != ModDimensions.THEDEEPERDARK_KEY && !destinationIsUG) {
            return null;
        }
        else {
    ...
    protected Optional<BlockUtil.FoundRectangle> getOrMakePortal(Entity entity, BlockPos pos) {
        ...
        if(existingPortal.isPresent()) {
            ...
            Direction.Axis portalAxis = this.level.getBlockState(entity.portalEntrancePos).getOptionalValue(DeeperDarkPortalBlock.AXIS).orElse(Direction.Axis.X);
